...
#if VERSIONMAC
        /*
         no effect; 'gs' seems to be a constant
         */
...
int IMImage2::write(PA_PluginParameters params, short index)
{
	...
	try { 
			Magick::Blob myblob;
			_image.write(&myblob);
			PA_SetBlobParameter(params, index, (void *)myblob.data(), (PA_long32)myblob.length());
			return 0;
		}
	catch(Magick::Exception &error_) 
	{
...
int IMImage2::GetProperty(int selector, C_TEXT &stringpara, double &para1, double &para2, double &para3, double &para4)
{
	...
	try { 
		...
		switch (selector)
		{
			...
			case IM_Pref_Magick:
				stringpara.setUTF8String((const uint8_t *)_image.magick().c_str(), (uint32_t)_image.magick().size());
				break;
			...
			case IM_Pref_BackgroundColor:
				...
				stringpara.setUTF8String((const uint8_t *)dummyColorStr.c_str(), (uint32_t)dummyColorStr.size());
				break;
